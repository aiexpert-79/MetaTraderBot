...
from main import trade_data

# dataframe = trade_data
# print("=============================>>",trade_data)
...
columns = [
    ...
    "ema_200", "ema_50", "ema_cross", "take_profit", "stop_price", "stop_loss"
]

array = np.random.rand(5, 14)
dataframe = np.array(array, dtype=list(zip(columns, ["float"] * len(columns))))
dataframe_columns = dataframe.dtype.names
...
def dataframe_to_table(dataframe, title):
    ...
    """
    # if dataframe is None or not isinstance(dataframe, pandas.DataFrame):
    ...
    # Create the figure
    fig = go.Figure(data=[go.Table(
        header=dict(values=list(dataframe_columns), align='left'),
        cells=dict(values=[dataframe[col] for col in dataframe_columns], align='left')
    )])
    # Add in the title
...
if __name__ == '__main__':
    # Example usage of the display_backtest function
    # Create dummy figures for demonstration
    proposed_trades = go.Figure()
    completed_trades = go.Figure()
    win_objects = go.Figure()
    loss_objects = go.Figure()
    proposed_trades_table = go.Figure()

    # Call the display_backtest function
    display_backtest(proposed_trades, completed_trades, win_objects, loss_objects, proposed_trades_table)