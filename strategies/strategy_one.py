...
def get_and_transform_mt5_data(symbol, timeframe, number_of_candles, pip_size):
    ...
    raw_data = mt5_lib.query_historic_data(symbol, timeframe, number_of_candles)
...
def create_new_order(decision_outcome, candle_dataframe, pip_size, symbol):
    ...
    if decision_outcome == "DoNothing":
        ...
        comment = "Green Order"
        # Send order to place_order function in mt5_lib.py
        mt5_lib.place_order("BUY_STOP", symbol, 0.1, buy_stop, stop_loss, take_profit, comment)
        return
        ...
        comment = "Red Order"
        # Send order to place_order function in mt5_lib.py
        mt5_lib.place_order("SELL_STOP", symbol, 0.1, buy_stop, stop_loss, take_profit, comment)
        return
...
def update_trailing_stop(order, trailing_stop_pips, pip_size):
    ...
    if order[12] > order[11]:
        ...
        if new_stop_loss > order[11]:
            ...
            st.write(new_take_profit)
            # Send order to modify_position
            mt5_lib.modify_position(order_number=order_number, symbol=symbol, new_stop_loss=new_stop_loss,
