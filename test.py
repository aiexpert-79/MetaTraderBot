...
# Create the tabs
tabs = ["Main Trading", "Articles", "History"]
selected_tabs = st.multiselect("Select tabs to run", tabs, default=tabs[0])

if "Main Trading" in selected_tabs:
    # Code for Main Trading tab
    tab1, tab2, tab3 = st.columns([1, 1])
    ss = get_symbol_names()
    with tab1:
        symbol = st.selectbox('Choose stock symbol', options=ss, index=1)
    with tab2:
        timeframe = st.selectbox('Choose time frame', options=TIMEFRAMES, index=1)
...
if "Articles" in selected_tabs:
    # Code for Articles tab
    with st.tab("Articles"):
        st.header("A dog")
        try:
            st.text("owl")
